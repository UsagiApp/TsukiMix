...
fun PreferenceStore.getLongArray(key: String, defaultValue: List<Long>): Preference<List<Long>> =
	getObjectFromString(
		key,
		defaultValue,
		serializer = { it.joinToString(",") },
		deserializer = { it.split(",").mapNotNull(String::toLongOrNull) },
	)
...
inline fun <reified T : Enum<T>> PreferenceStore.getEnum(key: String, defaultValue: T): Preference<T> =
	getObjectFromString(
		...
		defaultValue,
		serializer = { it.name },
		deserializer = { runCatching { enumValueOf<T>(it) }.getOrDefault(defaultValue) },
	)

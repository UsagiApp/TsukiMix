...
internal class DirectDexClassLoader(dexPath: String, optDir: String, libPath: String?, parent: ClassLoader) :
	...
	ZipFile(input).use { zip ->
		...
		if (nested != null) {
			...
			return@use ZipFile(output).use {
				it.getEntry("AndroidManifest.xml") != null && it.entries().asSequence().any { e -> e.name.matches(DEX_REGEX) }
